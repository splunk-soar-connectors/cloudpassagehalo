...
def _parsed_data(response, provides):
    if provides == "check all servers for user":
        for curr_res in response:
            if curr_res['last_login_at']:
                curr_res["last_login_at"] = _get_time_label(curr_res['last_login_at'], True)
    if provides in ["check server for package", "check server for all packages"]:
        for curr_res in response:
            ...
            if curr_res.get("install_date"):
                try:
                    ...
                    pass
    if provides in ["check all servers for package", "check all servers for vulnerability",
                    "check all servers for process", "check all servers for user"]:
...
def _parse_server_data(response, provides):
    if provides == "get server information":
...
def _get_time_label(value, isago):
    ...
    if int(years):
        ...
        if years > 1:
            timelabel = "{years} years".format(years=years) + ago
        else:
            timelabel = "about a year" + ago
    else:
        months = epochdiff / 2628000
        if int(months):
            months = int(round(months))
            if months > 1:
                timelabel = "{months} months".format(months=months) + ago
            else:
                timelabel = "about a month" + ago
        else:
            days = epochdiff / 86400
            if int(days):
                days = int(round(days))
                if days > 1:
                    timelabel = "{days} days".format(days=days) + ago
                else:
                    timelabel = "a day" + ago
            else:
                ...
                if int(hours):
                    ...
                    if hours > 1:
                        timelabel = "{hours} hours".format(hours=hours) + ago
                    else:
                        timelabel = "about an hour" + ago
                else:
                    minutes = epochdiff / 60 % 60
                    if int(minutes):
                        minutes = int(round(minutes))
                        if minutes > 1:
                            timelabel = "{minutes} minutes".format(minutes=minutes) + ago
                        else:
                            timelabel = "about a minute" + ago
                    else:
                        timelabel = "less than a minute" + ago
    return timelabel


# Function that change custom response for all processes and for process
def parse_process_details(data):
    for item in data:
...
def parse_response(data):
    ...
    account['groups'] = account['groups'].split(', ')
...
# Function that change custom response for all users
...
def _get_ctx_result(result, provides):
    ...
    if provides == "get server information":
        ctx_result["data"] = _parse_server_data(data[0], provides)
    elif provides in ["check server for all processes", "check server for process"]:
        ctx_result["data"] = parse_process_details(data)
    elif provides in ["check all servers for user", "check all servers for package",
                      "check all servers for vulnerability", "check all servers for process",
                      "check server for package", "check server for all packages"]:
        ctx_result["data"] = _parsed_data(data, provides)
    elif provides == 'check server for user':
        ctx_result["data"] = parse_response(data[0])
    elif provides == 'check server for all users':
...
# Function to provide custom view for 'get server information' and 'check all servers for package' action
def display_details(provides, all_app_runs, context):
    ...
    if provides in ["check all servers for package", "check all servers for user",
                    "check all servers for vulnerability", "check all servers for process"]:
        return_page = 'cloudpassagehalo_servers_information.html'
    elif provides in ["check server for package", "check server for all packages"]:
        return_page = 'cloudpassagehalo_package_information.html'
    elif provides in ["check server for vulnerability", "check server for all vulnerabilities"]:
        return_page = 'cloudpassagehalo_vulnerability_details.html'
    elif provides in ["check server for process", "check server for all processes"]:
        return_page = 'cloudpassagehalo_display_processes.html'
    elif provides == "check server for user":
        return_page = 'cloudpassagehalo_display_user_details.html'
    elif provides == "check server for all users":
        return_page = 'cloudpassagehalo_display_users.html'
    else:
        return_page = 'cloudpassagehalo_server_information.html'
